   ...
   if (typeof POST['purchase_submit_button'] != 'undefined') {
       ...
       for (i in products) {
           ...
           if (isNonNegInt(q) == true) {
               response.redirect('invoice.html?' + qString); // Redirects to Invoice with query strings
           } else {
               response.redirect('product_display.html?' + qString); // Redirects back to Products page
           }
        }
      }
}